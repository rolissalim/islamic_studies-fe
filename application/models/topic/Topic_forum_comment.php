...
class Topic_forum_comment extends Api_response {

    function __construct() {
        parent::__construct();
        $this->_api_host = config_item('app_api_topic_forum');
     ...
     * @return type
     */
    public function get_forums($params, $start = 0, $rows = 15, $keyword = NULL, $period = NULL) {
        $url = $this->_api_host . "/get_forum_comments";
        $query = array(
            'start' => $start,
            'row' => $rows
        );
    ...
    public function get_check_report_resume($topic_id, $media, $sdate, $edate, $timeframe) {
        // Bandung Resume Forum Comment monthly report (17 July 2017 until 15 August 2017).xlsx
        $return = array();
        $file_type = ".xlsx";
